...
kleidicv_error_t rgb_to_yuv420sp_stripe_u8(
  ...
  switch (color_format) {
    case KLEIDICV_BGR_TO_NV21: {
      return RGBxorBGRxToYUV420<false, false, true>::rgb2yuv420_operation(
          src, src_stride, y_dst, y_stride, uv_dst, uv_stride, width, height,
          true, begin, end);
    }

    case KLEIDICV_RGB_TO_NV21: {
      return RGBxorBGRxToYUV420<false, true, true>::rgb2yuv420_operation(
          src, src_stride, y_dst, y_stride, uv_dst, uv_stride, width, height,
          true, begin, end);
    }

    case KLEIDICV_BGRA_TO_NV21: {
      return RGBxorBGRxToYUV420<true, false, true>::rgb2yuv420_operation(
          src, src_stride, y_dst, y_stride, uv_dst, uv_stride, width, height,
          true, begin, end);
    }

    case KLEIDICV_RGBA_TO_NV21: {
      return RGBxorBGRxToYUV420<true, true, true>::rgb2yuv420_operation(
          src, src_stride, y_dst, y_stride, uv_dst, uv_stride, width, height,
          true, begin, end);
    }

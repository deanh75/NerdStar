...
class RGB2YUVThread : public testing::TestWithParam<P> {
 public:
  ...
  void check(SingleThreadedFunc single_threaded_func,
    ...
    test::Array2D<uint8_t> src(size_t{width} * channels, height);
    test::Array2D<uint8_t> dst_single(size_t{width} * 3, height);
    test::Array2D<uint8_t> dst_multi(size_t{width} * 3, height);
...
TEST_P(RGB2YUVThread, RGB_TO_YUV444) {
  check(kleidicv_rgb_to_yuv_u8, kleidicv_thread_rgb_to_yuv_u8,
        KLEIDICV_RGB_TO_YUV444);
}

TEST_P(RGB2YUVThread, RGBA_TO_YUV444) {
  check(kleidicv_rgb_to_yuv_u8, kleidicv_thread_rgb_to_yuv_u8,
        KLEIDICV_RGBA_TO_YUV444);
}

TEST_P(RGB2YUVThread, BGR_TO_YUV44) {
  check(kleidicv_rgb_to_yuv_u8, kleidicv_thread_rgb_to_yuv_u8,
        KLEIDICV_BGR_TO_YUV444);
}

TEST_P(RGB2YUVThread, BGRA_TO_YUV444) {
  check(kleidicv_rgb_to_yuv_u8, kleidicv_thread_rgb_to_yuv_u8,
        KLEIDICV_BGRA_TO_YUV444);
}

TEST_F(RGB2YUVThread, ReturnsErrorForUnsupportedCombinations) {
  run_unsupported(kleidicv_thread_rgb_to_yuv_u8, KLEIDICV_RGB_TO_YUV444);
  run_unsupported(kleidicv_thread_rgb_to_yuv_u8, KLEIDICV_RGBA_TO_YUV444);
  run_unsupported(kleidicv_thread_rgb_to_yuv_u8, KLEIDICV_BGR_TO_YUV444);
  run_unsupported(kleidicv_thread_rgb_to_yuv_u8, KLEIDICV_BGRA_TO_YUV444);
}

INSTANTIATE_TEST_SUITE_P(, RGB2YUVThread,
